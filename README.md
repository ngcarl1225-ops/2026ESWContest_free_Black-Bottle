...
위험해지는 방향(정상→주의→경보)은 안전을 위해 항상 즉시 반영되지만, 완화되는 방향(경보→주의→정상)은
`FIRE_DEBOUNCE_SECONDS`(기본 3초) 동안 그 값이 유지돼야 반영된다 — 라이터 불꽃처럼 순간적으로 깜빡이는
신호에 화면/사이렌이 계속 껐다 켜졌다 하는 걸 막기 위함.
